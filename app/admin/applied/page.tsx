...
export default async function AppliedPage() {
  ...
  const { data: candidates, error } = await supabase
    .from('candidates')
    .select('id, name, email, resume_url, job_id, created_at')
  ...
  // Generate signed URLs (1hr) for all resumes
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((c, i) => (
                <tr
                  key={c.id}
                  className={`hover:bg-gray-50/40 transition-colors ${i < rows.length - 1 ? 'border-b border-gray-50' : ''}`}
                >
